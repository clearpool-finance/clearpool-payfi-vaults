...
| # | Item | Status | Notes |
|---|---|---|---|
| 1 | Mainnet-fork test for `EtherFiLiquid1Migration.t.sol` | **Open — needs `$MAINNET_RPC`** | Sandbox cannot fork mainnet. Run before deploy: `forge test --match-path "test/EtherFiLiquid1Migration.t.sol" --fork-url $MAINNET_RPC -vvv`. Test was repointed at `AtomicSolverV5` in `aae8854`; non-fork tests are 146/146 green. Fork test not exercised in CI. |
| 2 | Full deploy script dry-run on a fork | **Open — needs `$MAINNET_RPC`** | Sequence: `DeployAtomicSolverV5` → `ConfigureAtomicRoles` → `08_SetAuthorityAndTransferOwnerships` → `CheckAuthConfiguration`. The check script must pass every assertion against forked state before any mainnet broadcast. |
